...
class QualityIssue(Document):
	...
	def on_update(self):
		...
		if self.workflow_state == 'Pending for Physical Verification Officer Approval':
			...
			msg=emailMessage
			frappe.sendmail(subject="Quality Issue registered: {}: {}".format(self.customer_code, self.customer_name), content=msg,recipients = '{}'.format(requestorSalesTeam), cc = '{},{}'.format(rsmEmail, asmEmail),expose_recipients="header", sender="Notification@hectorbeverages.com")
			print("\n email sent \n")
		...
		if self.workflow_state == 'Rejected by Physical Verification Officer':
			...
			Regards,<br>
			Hector Beverages""".format(self.customer_name, self.customer_code, self.customer_location, self.customer_phone_number, self.type_of_issue, self.reason_of_rejection)
			frappe.sendmail(subject="Quality Issue Rejected: {}: {}".format(self.customer_code, self.customer_name), content=msg, cc = '{},{}'.format(rsmEmail, asmEmail), recipients = '{}'.format(requestorSalesTeam), expose_recipients="header", sender="Notification@hectorbeverages.com")
			print("\n email sent \n")
		...
		if self.workflow_state == 'Pending for Finance Team Approval':
			msg=emailMessage
			frappe.sendmail(subject="Quality Issue for your review: {}: {}".format(self.customer_code, self.customer_name), content=msg, recipients = '{}'.format(financeTeamEmail),sender="Notification@hectorbeverages.com")
			print("\n email sent \n")
		...
		if self.workflow_state == 'RCA Approved':
			...
			msg=emailMessage
			frappe.sendmail(subject="Quality Issue Completed: {}: {}".format(self.customer_code, self.customer_name), content=msg, cc = '{},{}'.format(rsmEmail, asmEmail), recipients = '{}'.format(requestorSalesTeam), expose_recipients="header",sender="Notification@hectorbeverages.com")
			print("\n email sent \n")
